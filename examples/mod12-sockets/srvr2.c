 ...
 * Server reads a string from client and sends back 
 * a greeting appended to the string */
...
int main() {
  ...
  while(1){
    if( recvfrom(sd, (char*)&buf, BUFSZ, 0,
	(struct sockaddr*)&client, &namesize)  == -1) { 
      perror("srvr2.c:main:recvfrom"); 
      exit(1); 
    }
    strcat(buf, ", Hello from the server" );

    if( sendto(sd, buf, BUFSZ, 0, 
	(struct sockaddr*)&client, namesize)  == -1) { 
